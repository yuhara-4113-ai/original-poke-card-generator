...
4. カード単体は**高解像度PNGをダウンロード**、待ち受けには**スマホ壁紙用画像をダウンロード**から保存します。
...
プレビュー中のSVGを複製し、外部参照している属性アイコンをData URLとして埋め込んでからCanvasへ描画します。カード単体は2倍の `1320 × 1842`、スマホ壁紙は時計用の余白を設けた `1440 × 3120` で生成します。壁紙の背景色はカードのタイプに合わせ、光沢を含める場合はカードのCanvas上へ光沢を合成します。
