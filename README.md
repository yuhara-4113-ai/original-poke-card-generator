...
| 壁 | **スマホ壁紙出力** | カードを時計用の余白付き1440 × 3120 px画像として保存します |
...
4. 待ち受けにする場合は、壁紙プレビュー内のカードを上下にドラッグするかスライダーで縦位置を調整します。
5. カード単体は**高解像度PNGをダウンロード**、待ち受けには**スマホ壁紙用画像をダウンロード**から保存します。
...
CardArtwork が SVG を再描画
    ↓ ref で同じ SVG を取得
DownloadButton がカード単体またはスマホ壁紙の PNG に変換
```
...
| `src/components/DownloadButton.jsx` | SVGをCanvasへ描画し、カード単体または時計用余白付きのスマホ壁紙PNGを保存 |
...
プレビュー中のSVGを複製し、外部参照している属性アイコンをData URLとして埋め込んでからCanvasへ描画します。カード単体は2倍の `1320 × 1842`、スマホ壁紙は時計用の余白を設けた `1440 × 3120` で生成します。壁紙内のカードはプレビュー上のドラッグまたはスライダーで安全な範囲内を上下に移動できます。壁紙の背景色はカードのタイプに合わせ、光沢を含める場合はカードのCanvas上へ光沢を合成します。
